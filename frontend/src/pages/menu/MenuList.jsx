...
const MenuList = () => {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert('Failed to upload image. Please try a smaller file (under 5MB) in JPG, PNG, or WEBP format.');
  ...
  const closeFormModal = () => {
    ...
    setEditItem(null);
    setImagePreview(null);
    setForm({ name: '', description: '', price: '', categoryId: '', inventoryItemId: '', isAvailable: true, imageUrl: '' });
  };
        ...
        <div className="menu-grid">
          {filtered.map(item => (
            <div key={item.id} className="menu-card">
              <div className="menu-card-img" style={!item.imageUrl ? { background: '#fef2f2', color: '#dc2626' } : { padding: 0, overflow: 'hidden' }}>
                {item.imageUrl ? (
                  <img
              ...
              <div className="menu-card-body">
                ...
                {item.inventoryItemName && (
                  <span className="menu-category" style={{ display: 'block', marginTop: 2, color: '#16a34a' }}>
                    📦 {item.inventoryItemName}
                  </span>
                )}
                ...
                <div className="menu-card-footer">
                  ...
                  <div className="menu-actions">
                    ...
                    <button className="btn-icon danger" onClick={() => setDeleteTarget(item)}>🗑️</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="modal-body">
              <div className="form-group">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                    ...
                    justifyContent: 'center', flexShrink: 0, border: '1px solid #e5e7eb'
                  }}>
                    {imagePreview ? (
              ...
              <div className="form-group">
                ...
                <select className="form-input" value={form.inventoryItemId} onChange={e => setForm({ ...form, inventoryItemId: e.target.value })}>
                ...
                <span style={{ fontSize: 12, color: '#6b7280', marginTop: 4, display: 'block' }}>
                  When this item is ordered, 1 unit will be deducted from the linked inventory per quantity ordered.
                </span>
              </div>
              <div className="form-group">
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn-primary" disabled={uploading}>Save</button>
              </div>
            </form>
          </div>
        </div>
      ...
      >
        <div className="confirm-dialog-content">
          ...
          <p className="confirm-message">
            <strong>"{deletedName}"</strong> has been successfully deleted from the menu.
          </p>
        </div>
      </Modal>
    </MainLayout>
