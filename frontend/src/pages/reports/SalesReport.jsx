...
const SalesReport = () => {
  ...
  const totalRevenue = dailySales.reduce((sum, d) => sum + (d.totalRevenue || 0), 0);
  const totalOrders = dailySales.reduce((sum, d) => sum + (d.totalOrders || 0), 0);
    ...
    <MainLayout>
      ...
      {showCustom && (
        <div className="custom-range">
          ...
          <button className="btn-primary" onClick={fetchReports} disabled={!customFrom || !customTo}>
            Apply
          </button>
        </div>
      )}
      ...
      <div className="stat-grid" style={{ marginBottom: 24 }}>
        <div className="stat-card">
          ...
          <div className="stat-info">
            <span className="stat-value">₱{totalRevenue.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
        ...
        <div className="reports-grid">
          ...
          <div className="report-card full-width">
            <div className="report-card-header">
              ...
              <button className="btn-export" onClick={() => exportCSV(dailySales, 'daily-sales')}>
                ⬇ Export CSV
              </button>
            </div>
            {chartData.length > 0 ? (
          ...
          <div className="report-card">
            <div className="report-card-header">
              ...
              <button className="btn-export" onClick={() => exportCSV(topItems, 'top-items')}>
                ⬇ Export
              </button>
            </div>
            {topItems.length > 0 ? (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={topItems} layout="vertical">
                  ...
                  <Bar dataKey="totalQuantitySold" fill="#dc2626" radius={[0, 6, 6, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
          ...
          <div className="report-card">
            <div className="report-card-header">
              ...
              <button className="btn-export" onClick={() => exportCSV(paymentSummary, 'payments')}>
                ⬇ Export
              </button>
            </div>
                ...
                <div className="payment-breakdown">
                  {paymentSummary.map((p, i) => (
                    <div key={p.method} className="payment-breakdown-row">
                      ...
                      <span className="payment-amount">₱{Number(p.totalAmount).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </>
                ...
                <tbody>
                  {dailySales.map((d, i) => (
                    <tr key={i}>
                      ...
                      <td>{d.totalOrders}</td>
                      <td><strong>₱{Number(d.totalRevenue).toFixed(2)}</strong></td>
                      <td>₱{d.totalOrders > 0 ? (d.totalRevenue / d.totalOrders).toFixed(2) : '0.00'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
